...
import AmbassadorModel from "../../models/acn/Ambassadors.js"
...
export const newTeam = async (req, res) => {
    ...
    try {
        ...
        const newMember = await AmbassadorModel.create({
            firstName,
            lastName,
            position,
            email,
...
export async function editeam(req, res) {
    ...
    try {
        const getTeamMember = await AmbassadorModel.findOne({ teamMemberId: id })
        if(!getTeamMember){
        ...
        const updateTeamMember = await AmbassadorModel.findByIdAndUpdate(
            getTeamMember?._id,
            {
                $set: {
                    firstName,
...
export async function getAdminAllTeam(req, res) {
    try {
        const getAllTeamMemebers = await AmbassadorModel.find().select('-_id')
...
export async function getAllTeam(req, res) {
    try {
        const getAllTeamMemebers = await AmbassadorModel.find({ active: true }).select('-_id')
...
export async function deleteTeamMember(req, res) {
    ...
    try {
        ...
        const deleteTeamMember = await AmbassadorModel.findOneAndDelete({ teamMemberId: id })
